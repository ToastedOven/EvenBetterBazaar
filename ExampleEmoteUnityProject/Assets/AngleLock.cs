...
public class AngleLock : MonoBehaviour
{
    ...
    void Update()
    {
        if (transform.localRotation.x > .8f || transform.localRotation.x < 0f)
        {
            ...
            Debug.Log($"--------------  [{aimConstraint.transform.localRotation.x}] [{aimConstraint.transform.localRotation.y}] [{aimConstraint.transform.localRotation.z}] [{aimConstraint.transform.localRotation.w}]");
